...
def subprocess_mount(*, source, destination, t):
    ...
    subprocess.run(
        ['mount', '-t', t, source, destination], capture_output=True
    )
...
@click.option(
    '--target',
    required=True,
    help='Target directory. For example: /mnt/chroot',
)
@click.option(
...
@click.option(
    '--enable-repo-thirdparty-brave',
    is_flag=True,
    help='Enable Brave repository.',
)
@click.option(
    '--enable-repo-thirdparty-nordvpn',
    is_flag=True,
    help='Enable NordVPN repository.',
)
@click.option(
    '--enable-repo-thirdparty-docker',
    is_flag=True,
    help='Enable Docker repository.',
)
@click.option(
...
@click.option(
    '--enable-repo-copr-golang', is_flag=True, help='Enable Go repository.'
)
@click.option(
    '--enable-repo-copr-sbcl', is_flag=True, help='Enable SBCL repository.'
)
@click.option(
    '--enable-repo-all', is_flag=True, help='Enable all repositories.'
)
def install(
    release,
    target,
    ...
    **kwargs,
    ...
    subprocess_mount(
        t='proc', source='proc', destination=os.path.join(target, 'proc')
    )

    subprocess_mkdir(os.path.join(target, 'sys'))
    subprocess_mount(
        t='sysfs', source='sysfs', destination=os.path.join(target, 'sys')
    )
    ...
    subprocess_mount(
        t='devtmpfs',
        source='devtmpfs',
        destination=os.path.join(target, 'dev'),
    )
    subprocess_mount(
        t='devpts',
        source='devpts',
        destination=os.path.join(target, 'dev', 'pts'),
    )
    ...
    subprocess_dnf(
        target, release, 'install', 'xfsprogs', 'dosfstools', 'btrfs-progs'
    )
    subprocess_dnf(
        target,
        release,
